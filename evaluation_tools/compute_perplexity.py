...
def main():
    ...
    logger.info(args)
    with open("papers_2021.txt", "r") as f:
        lines = f.readlines()
        gold_text = "\n\n".join(lines)
    max_length = model.config.n_positions
    stride = 1024
    encodings = tokenizer(gold_text, return_tensors="pt")
    nlls = []
    o_nlls = []
    ...
    for i in tqdm(range(0, encodings.input_ids.size(1), stride)):
        ...
        #nonstopwords_indexes = get_nonstopwords_indexes(tokenizer.convert_ids_to_tokens(input_ids.squeeze().tolist()), device=args.device)
        #filter_target_ids = input_ids.clone()[:, nonstopwords_indexes]
        #filter_target_ids[:, :-trg_len] = -100

        with torch.no_grad():
            outputs = model(input_ids, labels=target_ids, **model_kwargs)
            original_loss = outputs[0] * int(input_ids.size(1))
            lm_logits = outputs[1]
            ...
            # Flatten the tokens
            non_stop_labels_indexes = get_nonstopwords_indexes(tokenizer.convert_ids_to_tokens(shift_labels.squeeze().tolist()),device=args.device)
           ...
           # print(loss) 
            #neg_log_likelihood = loss * int(shift_logits.size(1))
            if torch.isnan(loss):
                continue
                #print(shift_labels.view(-1))
                #print(shift_logits.view(-1, shift_logits.size(-1)))
                print(tokenizer.convert_ids_to_tokens(input_ids.squeeze().tolist()))
                neg_log_likelihood = outputs[0] * int(input_ids.size(1))
            else:
                neg_log_likelihood = loss * int(shift_logits.size(1))
            #print(neg_log_likelihood)
            #filter_logits = lm_logits[:, nonstopwords_indexes, :]

            #shift_logits = lm_logits[..., :-1, :].contiguous()
            #shift_labels = filter_target_ids[..., 1:].contiguous()
            # Flatten the tokens
            #loss_fct = torch.nn.CrossEntropyLoss()
            #loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))
        ...
        o_nlls.append(original_loss)
        o_total = o_total + int(input_ids.size(1))
        total = total + int(shift_logits.size(1))
    #print(torch.stack(nlls).sum())
    ppl = torch.exp(torch.stack(nlls).sum()/total)
    o_ppl = torch.exp(torch.stack(o_nlls).sum()/o_total)
